export const Chiffres = () => {
  return (
    <>
      {/* L'avantage qu'aurais été l'utilisation du composant Section ici, c'est que tu n'aurais pas eu besoin de dupliquer la plupart des classes pour ta section, nous en parlerons demain ou à mon retour :D  */}
      <section className="flex flex-col items-center max-w-screen-2xl m-auto mt-[100px] gap-6">
        <h2 className="flex flex-col font-Arial text-center text-[20px]">
          Quelques chiffres clés
        </h2>

        {/* Si tu regarde le site en ligne, ici c'est plutôt un justify-between */}
        <div className="flex justify-around items-center">
          <div className="border-[2px] border-[#ada796] p-1 pl-10 rounded-[4px]">
            <h2 className="uppercase font-ChangeOne text-[20px]">
              D'expertises
            </h2>
          </div>

          {/* Pareil que pour les padding avec vh, mieux vaut éviter les margin en vw */}
          <div className="border-[2px] border-[#ada796] p-1 pl-10 rounded-[4px] mr-[5vw] ml-[10vw] text-end">
            <h2 className="uppercase font-ChangeOne text-[20px]">
              CHANTIERS RÉALISÉS
            </h2>
            <p className="uppercase font-ChangeOne text-[18px]">EN 2023</p>
          </div>

          <div className="border-[2px] border-[#ada796] p-1 pl-10 rounded-[4px] mr-[10vw] ml-[5vw] text-end">
            <h2 className="uppercase font-ChangeOne text-[20px]">CLIENTS</h2>
            <p className="uppercase font-ChangeOne text-[18px]">EN 2023</p>
          </div>

          <div className="border-[2px] border-[#ada796] p-1 pl-10 rounded-[4px]">
            <h2 className="uppercase font-ChangeOne text-[20px]">
              PARTENAIRES
            </h2>
          </div>
        </div>
      </section>
    </>
  );
};
